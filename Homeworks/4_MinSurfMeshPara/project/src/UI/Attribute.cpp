...
void Attribute::ComponentVisitor::ImplVisit(Ptr<CmptSimulate> simulate) {
	...
	grid->AddEditVal({ "stiff" }, simulate->GetStiff(), 1000.0f, [=](const float& val) {
		simulate->SetStiff(val);
		});
	grid->AddEditVal({ "mass" }, simulate->GetMass(), 1.0f, [=](const float& val) {
		simulate->SetMass(val);
		});
	grid->AddButton("set x min fix", [simulate]() {
		simulate->SetLeftFix();
		});
	grid->AddButton("set 2 Points fix", [simulate]() {
		simulate->Set2PointFix();
		});
	grid->AddButton("Implicit Euler Method", [simulate]() {
		simulate->SetSimulateType(SimulateType::ImplicitEuler);
		});
	grid->AddButton("Projective Dynamic Method", [simulate]() {
		simulate->SetSimulateType(SimulateType::ProjectiveDynamic);
		});
}
...
void Attribute::ComponentVisitor::ImplVisit(Ptr<TriMesh> mesh) {
	...
	grid->AddTitle("[ Fixed Boundary Parameterization ]");

	Grid::pSlotMap slotmap = std::make_shared<Grid::SlotMap>();
	(*slotmap)["None"] = [mesh, pOGLW = attr->pOGLW]() {};
	(*slotmap)["Uniform + Circle"] = [mesh, pOGLW = attr->pOGLW]()
	{
		auto paramaterize = Paramaterize::New(mesh);
		if (paramaterize->Run(1))
			printf("Paramaterize done\n");
		pOGLW->DirtyVAO(mesh);
	};
	(*slotmap)["Cotangent + Circle"] = [mesh, pOGLW = attr->pOGLW]()
	{
		auto paramaterize = Paramaterize::New(mesh);
		if (paramaterize->Run(2))
			printf("Paramaterize done\n");
		pOGLW->DirtyVAO(mesh);
	};
	(*slotmap)["Uniform + Square"] = [mesh, pOGLW = attr->pOGLW]()
	{
		auto paramaterize = Paramaterize::New(mesh);
		if (paramaterize->Run(3))
			printf("Paramaterize done\n");
		pOGLW->DirtyVAO(mesh);
	};
	(*slotmap)["Cotangent + Square"] = [mesh, pOGLW = attr->pOGLW]()
	{
		auto paramaterize = Paramaterize::New(mesh);
		if (paramaterize->Run(4))
			printf("Paramaterize done\n");
		pOGLW->DirtyVAO(mesh);
	};
	grid->AddComboBox("Parameterization Type", "None", slotmap);

	Grid::pSlotMap slotmap1 = std::make_shared<Grid::SlotMap>();
	(*slotmap1)["None"] = [mesh, pOGLW = attr->pOGLW]() {};
	(*slotmap1)["Uniform + Circle"] = [mesh, pOGLW = attr->pOGLW]()
	{
		auto paramaterize = Paramaterize::New(mesh);
		if (paramaterize->SetTexcoords(1))
			printf("SetTexcoords done\n");
		pOGLW->DirtyVAO(mesh);
	};
	(*slotmap1)["Cotangent + Circle"] = [mesh, pOGLW = attr->pOGLW]()
	{
		auto paramaterize = Paramaterize::New(mesh);
		if (paramaterize->SetTexcoords(2))
			printf("SetTexcoords done\n");
		pOGLW->DirtyVAO(mesh);
	};
	(*slotmap1)["Uniform + Square"] = [mesh, pOGLW = attr->pOGLW]()
	{
		auto paramaterize = Paramaterize::New(mesh);
		if (paramaterize->SetTexcoords(3))
			printf("SetTexcoords done\n");
		pOGLW->DirtyVAO(mesh);
	};
	(*slotmap1)["Cotangent + Square"] = [mesh, pOGLW = attr->pOGLW]()
	{
		auto paramaterize = Paramaterize::New(mesh);
		if (paramaterize->SetTexcoords(4))
			printf("SetTexcoords done\n");
		pOGLW->DirtyVAO(mesh);
	};
	grid->AddComboBox("Set Texcoords", "None", slotmap1);

	grid->AddTitle("[ ASAP ]");
	grid->AddButton("Show ASAP Parameterization", [mesh, pOGLW = attr->pOGLW]() {
		auto asap = ASAP::New(mesh);
		if (asap->ShowASAPPara())
			printf("ShowASAPPara() done\n");
		pOGLW->DirtyVAO(mesh);
	});
	grid->AddButton("Set ASAP Texcoords", [mesh, pOGLW = attr->pOGLW]() {
		auto asap = ASAP::New(mesh);
		if (asap->SetASAPTexcoords())
			printf("SetASAPTexcoords() done\n");
		pOGLW->DirtyVAO(mesh);
	});

	grid->AddTitle("[ ARAP ]");
	int* arap_iter_num = new(int);
	*arap_iter_num = 5;
	grid->AddEditVal("ARAP Iteration Number", *arap_iter_num, 1, 100, 99);
	grid->AddButton("Show ARAP Parameterization", [arap_iter_num, mesh, pOGLW = attr->pOGLW]() {
		auto arap = ARAP::New(mesh);
		if (arap->ShowARAPPara(*arap_iter_num))
			printf("showARAP done\n");
		pOGLW->DirtyVAO(mesh);
	});
	grid->AddButton("Set ARAP Texcoords", [arap_iter_num, mesh, pOGLW = attr->pOGLW]() {
		auto arap = ARAP::New(mesh);
		if (arap->SetARAPTexcoords(*arap_iter_num))
			printf("Set ARAPTexcoords done\n");
		pOGLW->DirtyVAO(mesh);
	});



	grid->AddTitle("[ other things ]");
