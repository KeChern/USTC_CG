...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "HW8 - displacement & normal", NULL, NULL);//窗口的宽，窗口的高，窗口标题
    if (window == NULL)
    {
    ...
    //gl::Texture2D checkerboard = loadTexture("../data/textures/Taylor.png");
    //gl::Texture2D normalmap = loadTexture("../data/textures/Taylor.jpg");
    //gl::Texture2D checkerboard = loadTexture("../data/textures/lyrics.png");
    //gl::Texture2D normalmap = loadTexture("../data/textures/lyrics.jpg");
...
void processInput(GLFWwindow *window)
{
    if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS)//Esc键
